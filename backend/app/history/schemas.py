...
class HistoryItemCreate(BaseModel):
    ...
    confidence: Optional[float] = Field(None, description="Overall verdict confidence float 0-1")
...
class DashboardStatistics(BaseModel):
    ...
    pass_count: int = Field(0, description="Evaluations with PASS verdict")
    needs_improvement_count: int = Field(0, description="Evaluations with NEEDS_IMPROVEMENT verdict")
    fail_count: int = Field(0, description="Evaluations with FAIL verdict")
    ...
    average_confidence: float = Field(0.0, description="Mean verdict confidence")
