...
def setup_mocks():
    ...
    yield mock_retrieval, mock_relevance
...
class TestEvaluationIntegration:
    def test_evaluate_relevance_integration_success(self, client, setup_mocks):
        """Verify successful relevance evaluation and preserved M1 retrieval behavior."""
        mock_retrieval, mock_relevance = setup_mocks
    ...
    def test_evaluate_relevance_integration_without_reference(self, client, setup_mocks):
        """Verify evaluation works without a reference answer."""
        mock_retrieval, mock_relevance = setup_mocks
    ...
    def test_evaluate_relevance_judge_unavailability_graceful(self, client, setup_mocks):
        """Verify Judge LLM unavailability returns relevance_evaluation=null gracefully."""
        mock_retrieval, mock_relevance = setup_mocks
        mock_relevance.evaluate_relevance.side_effect = JudgeLLMUnavailableError("Temporary limit.")
        ...
        # M1 integrity is preserved
        assert len(data["retrieved_chunks"]) == 1
        assert data["retrieved_chunks"][0]["id"] == "chunk_1"

        # Relevance evaluation is null (swallowed gracefully)
        assert data["relevance_evaluation"] is None

    def test_evaluate_relevance_judge_config_error_propagates(self, client, setup_mocks):
        """Verify Judge configuration errors propagate and cause 500 status code."""
        mock_retrieval, mock_relevance = setup_mocks
        mock_relevance.evaluate_relevance.side_effect = JudgeLLMConfigurationError("Bad credentials.")
